# Hi there! 👋 I'm Berk Gezgin

**AI Engineer · Computer Vision · Medical Imaging · Deep Learning**

> AI Engineer at **CranioCatch**, building end-to-end computer vision pipelines for clinical environments.  
> I work with DICOM, NIfTI, 2D radiographs, and intraoral images — from raw data to production-ready diagnostic tools actively used in real clinical settings.

---

## 🔬 What I Work On

- 3D/2D segmentation, detection & landmark detection on CT volumes and dental X-rays (Panoramic, Bitewing, Periapical, Lateral Cephalometric)
- Automated clinical report generation (paranasal airway volumetrics, TMJ assessment, implant evaluation, third molar classification)
- Bone loss quantification & clinically accurate periodontal measurement algorithms
- Facial soft tissue measurement pipelines from Smile / Rest / Profile photographs
- DICOM–STL registration workflows for anatomical consistency across modalities
- End-to-end automated training pipelines covering dataset preprocessing, train/validation/test split, model training, evaluation, and structured reporting — fully automated from raw data to final results

---

## 🛠 Tech Stack

**Training & Modeling**  
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![nnU-Net](https://img.shields.io/badge/nnU--Net-0A0A0A?style=flat)
![MONAI](https://img.shields.io/badge/MONAI-0099FF?style=flat)
![YOLOv8](https://img.shields.io/badge/YOLOv8%2Fv11%2Fv26-00FFFF?style=flat&logo=ultralytics&logoColor=black)
![Mask R-CNN](https://img.shields.io/badge/Mask%20R--CNN-764ABC?style=flat)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)

**Medical Data**  
![pydicom](https://img.shields.io/badge/pydicom-2E8B57?style=flat)
![NiBabel](https://img.shields.io/badge/NiBabel-2E8B57?style=flat)
![SimpleITK](https://img.shields.io/badge/SimpleITK-2E8B57?style=flat)
![Open3D](https://img.shields.io/badge/Open3D-2E8B57?style=flat)

**Vision & Processing**  
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat&logo=opencv&logoColor=white)
![Albumentations](https://img.shields.io/badge/Albumentations-CC0000?style=flat)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![SciPy](https://img.shields.io/badge/SciPy-8CAAE6?style=flat&logo=scipy&logoColor=white)
![scikit-image](https://img.shields.io/badge/scikit--image-F7931E?style=flat)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat)

**Languages & Tools**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## 📌 Featured Projects

| Project | Description | Stack |
|---------|-------------|-------|
| [YOLO-Pipeline-Auto](https://github.com/Cestovatels/YOLO-Pipeline-Auto) | End-to-end automated YOLO training pipeline with dataset preprocessing, training, evaluation & reporting | Python · YOLOv8 |
| [Vehicle-License-Plate-Recognition-System](https://github.com/Cestovatels/Vehicle-License-Plate-Recognition-System) | Complete ALPR system with detection, tracking, OCR & visualization | Python · OpenCV |

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Cestovatels&show_icons=true&theme=github_dark&hide_border=true&include_all_commits=true&count_private=true" height="160"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Cestovatels&layout=compact&theme=github_dark&hide_border=true&langs_count=6" height="160"/>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=Cestovatels&theme=github-dark-blue&hide_border=true&date_format=j%20M%5B%20Y%5D" height="150"/>
</p>

---

## 📫 Contact

- 📧 [berk_gezgin@hotmail.com](mailto:berk_gezgin@hotmail.com)
- 💼 [linkedin.com/in/berk-gezgin-7bg](https://linkedin.com/in/berk-gezgin-7bg)
- 📍 Eskişehir, Turkey

---

*Open to collaboration on computer vision & medical AI projects.*
